...
from worker.learningratefinder import LearningRateFinder
from worker.firedetectionnet import FireDetectionNet
...
def load_dataset(datasetPath):
    # grab the paths to all images in our dataset directory, then initialize
    # our lists of images
...
(train_x, test_x, train_y, test_y) = train_test_split(data, labels, test_size=config.TEST_SPLIT,
        random_state=42)

# initiliaze the trainingdataaugmentation object
...
# check to see if we are attemting to find an optimal learning rate
# before traing for the full number of epochs
if args["lr_find"] > 0:
    # initilize the learning rate finder and then train with learning rates
    # ranging from 1e-10 to 1e+1
    print("[INFO] finding learning rate...")
    lrf = LearningRateFinder(model)
    lrf.find(
        aug.flow(train_x, train_y, batch_size=config.BATCH_SIZE),
        1e-10, 1e+1,
        stepsPerEpoch=np.ceil((train_x.shape[0] / float(config.BATCH_SIZE))),
        epochs=5,
    ...
    # and then train the network for full set of epochs
...
predictions = model.predict(test_x, batch_size=config.BATCH_SIZE)
print(classification_report(test_y.argmzx(axis=1), 
