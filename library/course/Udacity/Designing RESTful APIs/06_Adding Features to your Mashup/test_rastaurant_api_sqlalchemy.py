...
try:
	print("\nAttempting Test 2: running all Restaurants...")
	url = address + "/restaurants"
	h = httplib2.Http()
	resp, result = h.request(url, "GET")
	if resp["status"] != "200":
		raise Exception("Receiced an unsuccessful status code of %s" % resp["status"])
...
# TEST THREE -- READ A SPECIFIC RESTAURANTS
try:
...
try:
	...
	print("Test 5 PASS: Succesfully delete first restaurant")
